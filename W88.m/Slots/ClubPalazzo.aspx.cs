            ...
            foreach (System.Xml.Linq.XElement xeCategory in xeCategories.Elements())
            {
                ...
                foreach (System.Xml.Linq.XElement xeGame in xeCategory.Elements())
                {
                    ...
                    if (commonCulture.ElementValues.getResourceString("PlayForFun", xeGame) == "true")
                    {
                        ...
                        sbGames.AppendFormat("<a class='btn-secondary' target='_blank' href=\"javascript:load_palazzo_link('{0}','{1}','{2}')\"><i class='icon-fullscreen'></i></a></div>", commonCulture.ElementValues.getResourceString("Type", xeGame), xeCategory.Name, "fun");
                    }
