...
public partial class Withdrawal_Default_app : BasePage
{
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        if (arrPending != null && arrPending.Length > 0)
        {
            ...
            //if (litScript != null) { litScript.Text += "<script type='text/javascript'>window.location.replace('/Withdrawal/Pending_app.aspx');</script>"; }
        }
    }
                ...
                if (!isProcessAbort)
                {
                    using (svcPayWithdrawal.WithdrawalClient svcInstance = new svcPayWithdrawal.WithdrawalClient())
                    {
                        ...
                        else
                        {
                            ...
                            {
                                strAlertCode = "-1";
                                strAlertMessage = string.Format("{0}\\n{1}", commonCulture.ElementValues.getResourceXPathString("/Withdrawal/InsufficientBalance", xeErrors), commonCulture.ElementValues.getResourceXPathString("/Withdrawal/error" + strTransferId, xeErrors));
                            }
                        }
    ...
    protected void DepositNav_btn_Click(object sender, EventArgs e)
    {
        Response.Redirect("/Deposit/Default_app.aspx");
    }
    protected void FundTranferNav_btn_Click(object sender, EventArgs e)
    {
        Response.Redirect("/FundTransfer/FundTransfer.aspx");
    }
}
