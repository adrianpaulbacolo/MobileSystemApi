    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            if (arrPendingTrx != null && arrPendingTrx.Length > 0)
            {
                ...
                {
                    sbPendingTrx.AppendFormat("<tr id='tr_{0}'><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td><a data-role='button' data-theme='a' href='javascript:void(0)' onclick='javascript:return CancelWithdrawal(this);' data-id='{0}' data-method='{5}'>{4}</a></td></tr>"
            ...
            {
                if (litScript != null) { litScript.Text += "<script type='text/javascript'>window.location.replace('/Withdrawal/Withrawal.aspx');</script>"; }
            }
        }
    }
}