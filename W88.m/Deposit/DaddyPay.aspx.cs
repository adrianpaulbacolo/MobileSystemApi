    ...
    private void InitiateDeposit(string selectedBank, string strDepositAmount)
    {
        using (svcPayDeposit.DepositClient client = new svcPayDeposit.DepositClient())
        {
            ...
            else
            {
                ...
                {
                    string config = commonEncryption.GetMd5Hash(commonEncryption.decrypting(ConfigurationManager.AppSettings["privateKey_daddyPay"]));
                    ...
                    string key = commonEncryption.GetMd5Hash(builder.ToString());
