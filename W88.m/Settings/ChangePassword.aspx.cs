...
namespace Settings
{
    public partial class SettingsChangePassword : BasePage
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            else if (commonValidation.isInjection(strPasswordNew) || strPasswordNew.Length < 8 || strPasswordNew.Length > 10)
            {
                strAlertMessage = commonCulture.ElementValues.getResourceXPathString("UpdatePassword/InvalidPassword",
                    xeErrors);
                isProcessAbort = true;
            }
            else if (commonValidation.isInjection(strPasswordConfirm) || strPasswordConfirm.Length < 8 || strPasswordConfirm.Length > 10)
            {
                strAlertMessage = commonCulture.ElementValues.getResourceXPathString("UpdatePassword/InvalidPassword",
                    xeErrors);
                isProcessAbort = true;
            ...
            if (!isProcessAbort)
            {
                try
                {
                    using (wsMemberMS1.memberWSSoapClient wsInstance = new wsMemberMS1.memberWSSoapClient())
                    {
                        ...
                        strProcessRemark =
                            string.Format(
                                "OperatorId: {0} | MemberId: {1} | Password: {2} | PasswordNew: {3} | REMOTEIP: {4} | FORWARDEDIP: {5} | REQUESTERIP: {6}",
