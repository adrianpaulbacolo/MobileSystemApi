...
namespace W88.BusinessLogic.Shared.Helpers
{
    public class ListOfValuesHelper : BaseHelper
    {
        ...
        public List<LOV> GetCurrencies(XElement element, out string selectedCurrency)
        {
            var currencies = new List<LOV>()
            {
                new LOV()
                {
                    Text = CultureHelpers.ElementValues.GetResourceString("drpCurrencySelect", element),
                    Value = "-1"
                }
            };

            var curr = GetCurrencySettings();
            foreach (var item in curr)
            {
                currencies.Add(new LOV()
                {
                    Text = CultureHelpers.ElementValues.GetResourceXPathString("Currency/" + item, element),
                    Value = item
                });
            }
            ...
            selectedCurrency = currencies.FirstOrDefault(x => x.Value == currency) != null ? selectedCurrency : "-1";
